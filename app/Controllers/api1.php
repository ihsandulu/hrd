...
?>
    ...
    public function inputabsen()
    {
        ...
        return $this->response->setJSON(['status' => $status, 'keterangan' => $data["message"]]);
    }
    ...
    public function simulasi()
    {
        ...
        $input["masterbpjs"] = $arbpjs;

        //user
        if ($user_id != "") {
            $user_id = $user_id;
            $user = " absen.user_id = " . $user_id;
            $anduser = " ";
        } else {
            $user = "";
            $anduser = "";
        ...
        //tahun
        $gaji_tahun = date("Y", strtotime($dari));
        $gaji_bulan = date("m", strtotime($dari));
        ...
        $sql = "SELECT *, 
                ...
                WHERE " . $user . $periode . " GROUP BY absen.user_id";
        // echo $sql;die;
        ...
        foreach ($absen->getResult() as $absen) {
            ...
            $input["gaji_bulan"] = $gaji_bulan;
            $input["gaji_tahun"] = $gaji_tahun;
    ...
    {
        $absen_date = $this->request->getGet("absen_date");
        $user_id = $this->request->getGet("user_id");
        $absen_note = $this->request->getGet("absen_note");
        //cari dulu di table absen
        $absen = $this->db->table("absen")
            ->where("absen_date", $absen_date)
            ->where("user_id", $user_id)
            ->get();
        if ($absen->getNumRows() > 0) {
            $usr = $absen->getRow();
            $input["absen_note"] = $absen_note;
            $where["absen_id"] = $usr->absen_id;
            $this->db->table("absen")->where($where)->update($input);
        } else {
            $input["absen_note"] = $absen_note;
    ...
    public function inputabsenmanual()
    {
        ...
        foreach ($user_ids as $uid) {
            ...
            //cek apakah tipe normal atau bukan
            if ($input["absen_type"] != "Normal") {
                $input["absen_masuk"] = "";
                $input["absen_keluar"] = "";
            }
            if ($input["absen_type"] != "Cuti") {
                $input["cuti_id"] = "";
            }
            if ($input["absen_type"] != "Izin") {
                $input["absen_note"] = "";
            }
            if ($input["absen_type"] != "Sakit") {
                $input["absen_skd"] = "";
            }
            if ($input["absen_type"] != "Alpha") {
            }
            //cek absen
            $cekcok["user_id"] = $input["user_id"];
            $cekcok["absen_date"] = $input["absen_date"];
    ...
    public function updateabsenmanual()
    {
        foreach ($this->request->getGet() as $e => $f) {
        ...
        $this->db->table('absen')->update($input, array("absen_id" => $this->request->getGet("absen_id")));
